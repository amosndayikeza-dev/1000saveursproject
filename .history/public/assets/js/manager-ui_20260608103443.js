        ...
        tbody.innerHTML = sales.map(sale => `
            <tr class="tft-b-bottom-gris">
                ...
                <td class="tft-title4 tft-clr-white3">
                    <button class="tft-btn-sm tft-bg-greensav" onclick="viewInvoice(${sale.sale_id})">
...
window.viewInvoice = async function(saleId) {
    try {
        ...
        inv.items.forEach(item => {
            itemsHtml += `
                <tr style="border-bottom:1px solid #eee;">
                    ...
                    <td style="padding:8px; text-align:right;">${Number(item.unit_price).toLocaleString('fr-FR')} FBU</td>
                    <td style="padding:8px; text-align:right;">${Number(item.lineTotal).toLocaleString('fr-FR')} FBU</td>
                </tr>
            `;
        });
        ...
        showManagerModal('Facture', contentHtml, null); // pas de confirmation
    } catch (err) {
        console.error(err);
