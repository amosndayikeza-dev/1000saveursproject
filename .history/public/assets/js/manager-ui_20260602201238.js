...
(function () {
     ...
     async function loadDebts() {
        ...
        try {
            ...
            tbody.innerHTML = debts.map(d => {
                const remaining = Number(d.remaining_amount) || (d.amount - (d.paid_amount || 0));
    ...
    async function handlePayDebt(event) {
        ...
        // Ouvrir une modale pour saisir le montant (ou utiliser prompt pour simplifier)
        const amount = prompt(`Montant à payer (max ${formatMoney(remaining)}) :`, remaining);
        if (!amount) return;
        try {
            await ManagerAPI.debts.pay(debtId, parseFloat(amount));
            showToast('Paiement enregistré');
            loadDebts(); // rafraîchir
        } catch (err) {
            showToast(err.message, true);
        }
    }
