...
window.viewInvoice = async function(saleId) {
    ...
    try {
        ...
        const inv = data.data;
        // Format date sans slash : JJ-MM-AAAA
        const formatDate = (d) => {
            if (!d) return '—';
        ...
        const generatedAt = new Date(inv.generatedAt).toLocaleString('fr-FR').replace(/\//g, '-');

        const deptName = inv.departement?.name || '—';
        const creatorName = inv.createdBy ? (inv.createdBy.first_name + ' ' + inv.createdBy.last_name) : '—';

        // Construction de la facture texte (style ticket XPRINTER)
        let lines = [];
        lines.push('='.repeat(48));
        lines.push('         1000 SAVEURS');
        lines.push('     Facture de vente');
        lines.push('='.repeat(48));
        lines.push(`N° : ${inv.invoiceNumber}`);
        lines.push(`Date : ${soldAt}`);
        lines.push(`Dept : ${deptName}`);
        lines.push(`Vendeur : ${creatorName}`);
        lines.push('-'.repeat(48));
        lines.push(`Désignation          Qté   PU      Total`);
        for (const item of inv.items) {
            const name = item.product_name.substring(0, 20).padEnd(20, ' ');
            const qty = String(item.quantity).padStart(4, ' ');
            const price = String(Number(item.unit_price).toFixed(0)).padStart(6, ' ');
            const total = String(Number(item.lineTotal).toFixed(0)).padStart(8, ' ');
            lines.push(`${name} ${qty} ${price} ${total}`);
        }
        lines.push('-'.repeat(48));
        lines.push(`Total HT : ${fmtMoney(inv.totalAmount)}`);
        lines.push(`TVA (${inv.taxRate}%) : ${fmtMoney(inv.taxAmount)}`);
        lines.push(`Total TTC : ${fmtMoney(inv.grandTotal)}`);
        lines.push('='.repeat(48));
        lines.push(`Généré le ${generatedAt}`);
        lines.push('='.repeat(48));
        lines.push('   Merci de votre visite !');
        lines.push('   Règlement à la caisse');
        lines.push('='.repeat(48));

        const textContent = lines.join('\n');
        // Affichage dans une modale avec une police monospace
        const contentHtml = `<pre style="font-family: monospace; font-size: 12px; background: white; color: black; padding: 15px; border-radius: 5px; margin: 0;">${escapeHtml(textContent)}</pre>
        <div style="text-align: center; margin-top: 15px;">
            <button class="tft-btn tft-bg-greensav" onclick="window.print()">Imprimer</button>
        </div>`;
