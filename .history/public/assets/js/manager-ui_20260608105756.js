...
(function () {
    ...
    window.showAddSaleModal = async function() {
        ...
        showManagerModal('Nouvelle vente', formHtml, async () => {
            ...
            const paidAmount = document.getElementById('modal-sale-paid-amount').value;
    ...
    async function handleAdjustStock(event) {
        ...
        const productId = parseInt(btn.dataset.productId , 10); // ← important
        if (isNaN(productId)) {
            showToast('ID produit invalide', true);
            return;
        }
        let productName = '';
        try {
        ...
        openAdjustStockModal(productId, productName); // ← passer l'ID
    }
    ...
    const formHtml = `
        <div class="tft-form-group">
            <label>Produit</label>
            <input type="text" class="tft-form-control" value="${escapeHtml(productName)}" disabled>
        </div>
        <div class="tft-form-group">
            <label>Type de mouvement</label>
            <select id="adjust-type" class="tft-form-control" required>
        ...
        <div class="tft-form-group">
            <label>Quantité</label>
            <input type="number" id="adjust-quantity" class="tft-form-control" min="1" required>
        </div>
        <div class="tft-form-group">
            <label>Raison (optionnelle)</label>
            <input type="text" id="adjust-reason" class="tft-form-control" placeholder="Ex: réapprovisionnement, casse, vente...">
        </div>
    `;

    showManagerModal('Ajuster le stock', formHtml, async () => {
        ...
        try {
            const result = await ManagerAPI.stock.adjust({
                productId: productId,
                type: type,
                quantity: quantity,
                reason: reason
            ...
            await renderStockState(); // Recharge la liste
        } catch (err) {
            console.error(err);
            showToast(err.message, true);
            throw err; // Empêche la fermeture de la modale
        }
    });
}
   ...
   async function handlePayDebt(event) {
    ...
    // Utiliser fmtMoney (déjà définie dans le fichier) pour formater le montant
    const amount = prompt(`Montant à payer (max ${fmtMoney(remaining)}) :`, remaining);
    if (!amount) return;
    try {
        await ManagerAPI.debts.pay(debtId, parseFloat(amount));
        showToast('Paiement enregistré');
        await loadDebts(); // recharger le tableau
    } catch (err) {
        showToast(err.message, true);
    }
}
    ...
    function openPaymentModal(debtId, remaining) {
        const formHtml = `
            <div class="tft-form-group">
                <label>Montant à payer (max ${fmtMoney(remaining)})</label>
                <input type="number" id="payment-amount" class="tft-form-control" step="any" max="${remaining}" value="${remaining}">
            </div>
            <div class="tft-form-group">
                <label>Mode de paiement</label>
                <select id="payment-method" class="tft-form-control">
        ...
        `;
        showManagerModal('Encaissement d’une dette', formHtml, async () => {
            ...
            try {
                await ManagerAPI.debts.pay(debtId, amount, method);
                showToast('Paiement enregistré');
                await loadDebts(); // rafraîchir
            } catch (err) {
                showToast(err.message, true);
                throw err;
            }
...
async function loadProducts(page = 1) {
    ...
    try {
        ...
        currentProductPage = pagination.currentPage || page;
        ...
        tbody.innerHTML = products.map(p => {
            ...
            return `
                <tr class="tft-b-bottom-gris">
                    ...
                    <td class="tft-title4 tft-clr-white3">${escapeHtml(p.name)}</div></td>
...
function updateProductPagination() {
    ...
    for (let i = 1; i <= totalProductPages; i++) {
        ...
        li.className = `tft-icon-carre-petit tft-clr-white3 tft-br-none ${i === currentProductPage ? 'tft-active-page' : ''}`;
        li.textContent = i;
    ...
    if (prevBtn) prevBtn.onclick = () => { if (currentProductPage > 1) loadProducts(currentProductPage - 1); };
    if (nextBtn) nextBtn.onclick = () => { if (currentProductPage < totalProductPages) loadProducts(currentProductPage + 1); };
}

function bindProductActions() {
    const addBtn = document.getElementById('add-product-btn');
    if (addBtn) {
        addBtn.removeEventListener('click', () => showProductModal(false));
        addBtn.addEventListener('click', () => showProductModal(false));
    }
}
...
async function handleEditProduct(event) {
    ...
    try {
        const res = await ManagerAPI.products.get(id);
        if (res.success) showProductModal(true, res.data);
...
async function handleDeleteProduct(event) {
    ...
    function showProductModal(isEdit = false, product = null) {
        ...
        const formHtml = `
            <div class="tft-form-group">
                <label>Nom *</label>
                <input type="text" id="prod-name" class="tft-form-control" value="${escapeHtml(product?.name || '')}" required>
            </div>
            <div class="tft-form-group">
                <label>Description</label>
                <textarea id="prod-description" class="tft-form-control" rows="3">${escapeHtml(product?.description || '')}</textarea>
            </div>
            <div class="tft-form-group">
                <label>Prix unitaire (FBU) *</label>
                <input type="number" id="prod-price" class="tft-form-control" step="any" value="${product?.unit_price || ''}" required>
            </div>
            <div class="tft-form-group">
                <label>Stock initial *</label>
                <input type="number" id="prod-stock" class="tft-form-control" value="${product?.current_stock || 0}" disabled>
            </div>
            <div class="tft-form-group">
                <label>Seuil d'alerte</label>
                <input type="number" id="prod-threshold" class="tft-form-control" value="${product?.low_stock_threshold || 5}">
            </div>
        `;
        showManagerModal(title, formHtml, async () => {
            ...
            if (!name || isNaN(price) || price <= 0 || isNaN(stock) || stock < 0) {
                showToast('Veuillez remplir correctement les champs obligatoires', true);
                return;
            }
            const payload = { name, description, unitPrice: price, currentStock: stock, lowStockThreshold: threshold };
            try {
                if (isEdit && product?.id) {
                    await ManagerAPI.products.update(product.id, payload);
                    showToast('Produit modifié');
                } else {
                    await ManagerAPI.products.create(payload);
                    showToast('Produit ajouté');
                }
                await loadProducts(currentProductPage);
            } catch (err) {
...
window.viewInvoice = async function(saleId) {
    ...
    const showToastMsg = (msg, isError) => {
    ...
    try {
        ...
        if (data.status !== 'success') {
            showToastMsg(data.message || 'Impossible de charger la facture', true);
            return;
        }
        const inv = data.data;
        const soldAt = formatDate(inv.soldAt);
        ...
        if (inv.items && inv.items.length) {
            inv.items.forEach(item => {
                itemsHtml += `
                    <tr class="tft-b-bottom-gris">
                        <td class="tft-title4 tft-clr-white3">${escapeHtml(item.product_name)}</div></td>
                        <td class="tft-title4 tft-clr-white3" style="text-align:center;">${item.quantity}</div></td>
                        <td class="tft-title4 tft-clr-white3" style="text-align:right;">${fmtMoney(item.unit_price)}</div></td>
                        <td class="tft-title4 tft-clr-white3" style="text-align:right;">${fmtMoney(item.lineTotal)}</div></td>
                    </tr>
                `;
            });
        } else {
            itemsHtml = '<tr><td colspan="4" class="mgr-empty-hint">Aucun article</td></tr>';
        }

        const contentHtml = `
            <div style="background: var(--tft-black2, #1a1d24); color: var(--tft-white3, #e2e8f0); padding: 10px;">
                <h2 class="tft-title1" style="color: var(--tft-greensav, #22c55e); border-bottom: 2px solid #22c55e; padding-bottom: 8px;">
                    FACTURE ${escapeHtml(inv.invoiceNumber)}
                </h2>
                <div style="margin: 15px 0;">
                    <p><strong class="tft-title4">Date de vente :</strong> ${soldAt}</p>
                    <p><strong class="tft-title4">Département :</strong> ${escapeHtml(deptName)}</p>
                    <p><strong class="tft-title4">Vendeur :</strong> ${escapeHtml(creatorName)}</p>
                </div>
                <table class="tft-table" style="width:100%; border-collapse: collapse;">
                    <thead>
                        <tr class="tft-b-bottom-gris">
                            <th class="tft-title3">Désignation</th>
                            <th class="tft-title3" style="text-align:center;">Qté</th>
                            <th class="tft-title3" style="text-align:right;">Prix unitaire</th>
                            <th class="tft-title3" style="text-align:right;">Total</th>
                        </tr>
                    </thead>
                    ...
                    <tfoot>
                        <tr class="tft-b-bottom-gris">
                            ...
                            <td style="text-align:right;">${fmtMoney(inv.totalAmount)}</div></td>
                        </tr>
                        <tr class="tft-b-bottom-gris">
                            <td colspan="3" style="text-align:right;"><strong>TVA (${inv.taxRate || 0}%) :</strong></td>
                            <td style="text-align:right;">${fmtMoney(inv.taxAmount || 0)}</div></td>
                        </tr>
                        <tr style="color: var(--tft-orangesav, #f97316);">
                            <td colspan="3" style="text-align:right;"><strong>Total TTC :</strong></td>
                            <td style="text-align:right;"><strong>${fmtMoney(inv.grandTotal)}</strong></div></td>
                        </tr>
                    </tfoot>
                </table>
                <p style="margin-top: 20px; font-size: 12px; color: #9ca3af;"><em>Facture générée le ${generatedAt}</em></p>
                <div style="text-align: center; margin-top: 20px;">
                    <button class="tft-btn tft-bg-greensav" onclick="window.print()" style="color: white;">
        ...
        if (typeof window.showManagerModal === 'function') {
            window.showManagerModal('Facture', contentHtml, null);
        } else {
            alert(contentHtml);
        }
    } catch (err) {
        console.error(err);
        showToastMsg('Erreur lors du chargement de la facture', true);
    }
};