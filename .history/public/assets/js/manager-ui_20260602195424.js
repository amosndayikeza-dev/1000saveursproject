...
(function () {
    ...
    async function loadDebts() {
        ...
        if (!tbody) return;
        const statusFilter = document.getElementById('debts-status-filter')?.value || 'all';
        tbody.innerHTML = '<tr><td colspan="8" class="mgr-empty-hint">Chargement…</div></td>';
        try {
            const res = await ManagerAPI.debts.list(statusFilter);
            const debts = res.data || [];
            if (!debts.length) {
                tbody.innerHTML = '<tr><td colspan="8" class="mgr-empty-hint">Aucune dette</div></td>';
                return;
            }
            tbody.innerHTML = debts.map(d => {
                const remaining = d.amount - (d.paid_amount || 0);
                const canPay = d.status !== 'paid' && remaining > 0;
                return `
                    <tr class="tft-b-bottom-gris">
                        <td class="tft-title4 tft-clr-white3">${escapeHtml(d.debtor_name || d.client_name || '—')}</div></td>
                        <td class="tft-title4 tft-clr-white3">${escapeHtml(d.product_name || '—')}</div></td>
                        <td class="tft-title4 tft-clr-white3">${fmtMoney(d.amount)}</div></td>
                        <td class="tft-title4 tft-clr-white3">${fmtMoney(d.paid_amount || 0)}</div></td>
                        <td class="tft-title4 tft-clr-orangesav">${fmtMoney(remaining)}</div></td>
                        <td class="tft-title4 tft-clr-white3">${fmtDate(d.due_date)}</div></td>
                        <td class="tft-title4 tft-clr-white3">${d.status === 'paid' ? 'Soldée' : 'En cours'}</div></td>
                        <td class="tft-bdr-l-gris-1">${canPay ? `<button class="tft-btn-sm tft-bg-greensav pay-debt-btn" data-id="${d.id}" data-remaining="${remaining}">Encaisser</button>` : '—'}</div></td>
                    <tr>
                `;
            }).join('');
            ...
            console.error(err);
            tbody.innerHTML = `<tr><td colspan="8" class="mgr-empty-hint tft-clr-red">Erreur : ${escapeHtml(err.message)}</div></td>`;
        }
    }
    ...
    async function loadManagerPage() {
        ...
        try {
            ...
            switch (page) {
                ...
                case 'debts':
                    await loadDebts();
                      document.getElementById('debt-status-filter')?.addEventListener('change', () => loadDebts());document.getElementById('debt-status-filter')?.addEventListener('change', () => loadDebts());
                    break;
                case 'employees':
                    await loadEmployees();
                    break;
