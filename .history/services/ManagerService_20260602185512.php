...
class ManagerService {
     ...
     */
    public function recordPayment($departementId, $saleId, $amount, $method = 'cash') {
        // Vérifier que la vente appartient bien au département
        $sale = $this->salesDao->findById($saleId);
        if (!$sale || $sale['departement_id'] != $departementId) {
        ...
        // Recalculer le total payé et mettre à jour `sales.payment_status`
        $totalPaid = $this->db->fetchOne("SELECT SUM(amount) as total FROM payments WHERE sale_id = ?", [$saleId])['total'];
        $totalSale = $sale['total_amount'];
        $newStatus = ($totalPaid >= $totalSale) ? 'paid' : ($totalPaid > 0 ? 'partial' : 'unpaid');
        $this->db->update('sales', ['payment_status' => $newStatus], 'id = ?', [$saleId]);
        // Si vous utilisez une table debts, mettez‑la à jour ici
        return ['sale_id' => $saleId, 'total_paid' => $totalPaid, 'status' => $newStatus];
    }
}
