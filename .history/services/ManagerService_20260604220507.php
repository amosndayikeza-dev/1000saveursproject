...
class ManagerService {
   ...
   public function createSale($departementId, $userId, array $data, $paidAmount = 0) {
    ...
    if ($unitPrice <= 0) $unitPrice = (float)$product['unit_price'];
    ...
    if (strpos($soldAt, ' ') === false) $soldAt .= ' ' . date('H:i:s');
    ...
    try {
        ...
        // 3. Paiement immédiat (s’il existe)
        if ($paidAmount > 0) {
            $this->db->insert('payments', [
                'sale_id' => $saleId,
                'amount' => $paidAmount,
        ...
        // 4. Dette uniquement si reste à payer > 0
        if ($remaining > 0) {
            ...
            $this->debtsDao->create([
                ...
                'amount' => $remaining,                 // Montant restant dû
                'sale_item_id' => $saleItemId,
                'due_date' => $dueDate,
                'status' => 'pending',
                'paid_amount' => 0,                    // Aucun paiement sur cette dette
