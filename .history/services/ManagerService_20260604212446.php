...
class ManagerService {
    ...
    /**
 * Enregistre un paiement sur une dette.
 *
 * @param int      $debtId          ID de la dette
 * @param float    $paidAmount      Montant payé
 * @param int|null $departementId   Département du manager (optionnel, pour vérification)
 * @return array La dette mise à jour
...
public function recordDebtPayment($debtId, $paidAmount, $departementId = null) {
    ...
    // 2. Récupérer la ligne de vente (sale_item) associée
    $saleItem = $this->saleItemsDao->findById($debt['sale_item_id']);
    if (!$saleItem) {
        throw new InvalidArgumentException('Ligne de vente introuvable');
    }
    $saleId = $saleItem['sale_id'];

    // 3. Récupérer la vente
    $sale = $this->salesDao->findById($saleId);
    if (!$sale) {
        throw new InvalidArgumentException('Vente introuvable');
    }

    // 4. Vérifier l'appartenance au département (si fourni)
    ...
    try {
        ...
        // Insertion du paiement dans la table `payments`
        $this->db->insert('payments', [
            'sale_id'        => $saleId,
            'amount'         => $paidAmount,
            'payment_date'   => date('Y-m-d H:i:s'),
            'payment_method' => 'cash', // à rendre dynamique si besoin
        ]);

        // Recalcul du total payé pour la vente
        $totalPaidQuery = $this->db->fetchOne(
            "SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE sale_id = ?",
            [$saleId]
        );
        $totalPaid = (float)($totalPaidQuery['total'] ?? 0);
  ...
  public function createSale($departementId, $userId, array $data, $paidAmount = 0) {
    ...
    $remaining = round($totalAmount - $paidAmount, 2); // arrondi à 2 décimales
