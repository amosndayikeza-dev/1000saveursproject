...
class ManagerService {
    ...
    public function recordDebtPayment($debtId, $paidAmount, $departementId = null) {
        ...
        // Vérifier l’appartenance au département (via la vente associée)
        if ($departementId !== null) {
            $sale = $this->salesDao->findById($debt['sale_item_id'] ?? null);
            if (!$sale || $sale['departement_id'] != $departementId) {
                throw new InvalidArgumentException('Dette hors de votre département');
            }
        }

        $newPaid = ($debt['paid_amount'] ?? 0) + $paidAmount;
        $remaining = $debt['amount'] - $newPaid;
        $status = ($remaining <= 0) ? 'paid' : 'pending';

        // 2. Insérer le paiement dans la table `payments`
        $this->db->insert('payments', [
            'sale_id'      => $debt['sale_item_id'],   // ou l’ID de la vente si tu as `sale_id`
            'amount'       => $paidAmount,
            'payment_date' => date('Y-m-d H:i:s'),
            'payment_method' => 'cash',                // ou un paramètre venant de l’API
        ]);
        ...
        $this->debtsDao->update($debtId, [
            ...
            'status' => $status,
        'paid_at' => ($status === 'paid') ? date('Y-m-d') : null,
    ]);
