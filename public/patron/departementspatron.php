...
async function loadDepartements() {
    try {
        ...
        const response = await fetch('/1000saveursproject/api/index.php?resource=departements');
        ...
        console.log('Réponse API départements:', result);
        ...
        if (Array.isArray(result.data)) {
            ...
            // Si c'est un objet, essayons de le convertir en tableau
            console.warn('result.data est un objet, tentative de conversion...');
            departments = Object.values(result.data);
        } else {
            console.error('result.data n\'est pas un tableau:', result.data);
            departments = [];
        }
        
        console.log('Départements après traitement:', departments);
        
        const container = document.getElementById('container-departements');
        if (!container) {
            console.error('Conteneur container-departements non trouvé');
            return;
        }
        ...
        departments.forEach(dept => {
            ...
            const createdSource = dept.createdAt || dept.created_at || '';
            if (createdSource) {
                const match = createdSource.match(/(\d{4})-(\d{2})-(\d{2})/);
            ...
            card.innerHTML = `
                ...
                <div class="departement-btns">
                    <button class="tft-btn edit-btn" data-id="${dept.id}" data-name="${escapeHtml(dept.name)}" data-desc="${escapeHtml(dept.description || '')}" data-addr="${escapeHtml(dept.address || '')}" data-mgr="${dept.managerId || dept.manager_id || ''}">Editer</button>
        ...
        // Attacher les événements après avoir ajouté les éléments
        ...
        if (container) {
            container.innerHTML = `
                <div class="error">
                    ...
                    <p class="tft-sm-title1">${error.message}</p>
                </div>`;
        }
    }
}

// Fonction séparée pour attacher les événements
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            if (confirm('Supprimer définitivement ce département ?')) {
                try {
                    const res = await fetch(`/1000saveursproject/api/index.php?resource=departements&id=${encodeURIComponent(btn.dataset.id)}`, { method: 'DELETE' });
                    if (res.ok) {
        ...
        // ============================================================
        // CHARGER LES EMPLOYÉS POUR LE SELECT DES MANAGERS
       async function loadManagersSelect() {
    try {
        console.log('Chargement des employés...');
        const response = await fetch('/1000saveursproject/api/index.php?resource=employees');
        if (!response.ok) throw new Error('Erreur chargement employés');
        
        const result = await response.json();
        console.log('Réponse API employés:', result);
        
        if (!result.success) {
            throw new Error(result.message || 'API a retourné une erreur');
        }
        ...
        if (Array.isArray(result.data)) {
            ...
            employees = Object.values(result.data);
        } else {
            console.error('result.data n\'est pas un tableau:', result.data);
            employees = [];
        }
        
        console.log('Employés après traitement:', employees);
        
        const select = document.getElementById('manager-id');
        if (!select) {
            console.error('Select manager-id non trouvé');
            return;
        }
        ...
        if (employees.length > 0) {
            ...
            select.innerHTML += '<option value="" disabled>Aucun employé disponible</option>';
        }
    } catch (error) {
        console.error('Erreur chargement des employés:', error);
        const select = document.getElementById('manager-id');
        if (select) {
            select.innerHTML = '<option value="">Erreur chargement des employés</option>';
        }
    }
}
        ...
        document.getElementById('departementForm').addEventListener('submit', async (e) => {
            ...
            const payload = {
                ...
                manager_id: document.getElementById('manager-id').value ? parseInt(document.getElementById('manager-id').value) : null
            };

            const url = id
                ? `/1000saveursproject/api/index.php?resource=departements&id=${encodeURIComponent(id)}`
                : '/1000saveursproject/api/index.php?resource=departements';
